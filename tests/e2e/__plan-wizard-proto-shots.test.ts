...
test('A · Guided Stepper', async ({ page, login }) => {
	...
	await page.screenshot({ path: `${DIR}/A1-stepper-disciplines.png`, fullPage: true })

	await page.getByRole('button', { name: 'Next', exact: true }).click() // → experience
	await page.screenshot({ path: `${DIR}/A2-stepper-experience.png`, fullPage: true })
	...
	await page.screenshot({ path: `${DIR}/A3-stepper-review.png`, fullPage: true })

	await page.getByRole('button', { name: 'Generate plan' }).click({ force: true })
	await expect(page.getByRole('button', { name: /Approve/ })).toBeVisible({ timeout: 10_000 })
	await page.screenshot({ path: `${DIR}/A4-stepper-preview.png`, fullPage: true })
})
...
test('B · Narrative builder', async ({ page, login }) => {
	...
	await page.screenshot({ path: `${DIR}/B1-narrative-initial.png`, fullPage: true })

	await page.getByRole('button', { name: 'describe a goal' }).click()
	await page.getByPlaceholder(goalField).fill(GOAL)
	await page.screenshot({ path: `${DIR}/B2-narrative-editing.png`, fullPage: true })
	await page.getByText('Build me a').click() // close popover

	await page.getByRole('button', { name: 'Generate my plan' }).click({ force: true })
	await expect(page.getByRole('button', { name: /Approve/ })).toBeVisible({ timeout: 10_000 })
	await page.screenshot({ path: `${DIR}/B3-narrative-preview.png`, fullPage: true })
})

test('C · Split workbench', async ({ page, login }) => {
	await login()
	await page.goto('/training/plan/new?variant=C')
	await expect(page.getByText('Your plan preview will appear here')).toBeVisible()
	await page.screenshot({ path: `${DIR}/C1-workbench-empty.png`, fullPage: true })

	await page.getByPlaceholder(goalField).fill(GOAL)
	await page.getByRole('button', { name: 'Generate plan' }).click({ force: true })
	await expect(page.getByText('Plan preview')).toBeVisible({ timeout: 10_000 })
	await page.screenshot({ path: `${DIR}/C2-workbench-preview.png`, fullPage: true })
})
