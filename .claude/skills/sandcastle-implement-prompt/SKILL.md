...
description:
  Implements a single issue using the Sandcastle execution style with focused
  scope, TDD loop, validation commands, and structured completion signaling. Use
  when starting issue implementation from a sandcastle/issue-* branch.
---
...
- For UI work, use shadcn components/composition first; avoid custom inline
  components unless no shadcn option fits.
...
Pull issue details and comments with `gh issue view`. If there is a parent PRD,
include it for context. Only work on this issue. Work on branch {{BRANCH}}.
...
Use focused exploration. Pay attention to related tests. Use RGR if applicable:
...
If incomplete, leave a progress comment. Do not close the issue. When complete,
