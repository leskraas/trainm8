...
- Add a composite or single-column index that matches the loader’s `where` + `orderBy` (typically `userId` and `createdAt` for a user’s notes).
...
No Sly or `Icon` changes are required unless the loader UI is explicitly in scope.
