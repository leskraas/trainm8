...
4. Apply coding standards from `.sandcastle/CODING_STANDARDS.md`.
5. Re-run `npm run typecheck` and `npm run test`.
...
4. Apply maintainability refinements without behavior changes.
5. Run `npm run typecheck` and `npm run test` again.
6. Commit with `RALPH: Review - ...`.
