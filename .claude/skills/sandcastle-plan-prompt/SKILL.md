...
description:
  Generates a dependency-aware issue plan for Sandcastle-labeled issues and
  returns unblocked work as JSON. Use when planning issue execution, selecting
  parallelizable tasks, or creating sandcastle/issue-* branches.
---
...
An issue is unblocked if it has zero blocking dependencies. For each unblocked
issue, assign branch `sandcastle/issue-{number}-{slug}`. If a PRD has linked
implementation issues, treat that PRD as blocked.

# OUTPUT

Output JSON only inside <plan> tags. Include only unblocked issues. If all are
blocked, include the best single candidate.
```
