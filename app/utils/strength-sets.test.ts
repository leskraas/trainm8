...
const reps = (reps: string, load: Partial<DraftSetValue> = {}): DraftSetValue => ({
	kind: 'reps',
	reps,
	weightKg: '',
	pct1RM: '',
...
describe('setsAreUniform', () => {
	test('equal rep sets with equal loads are uniform', () => {
		expect(
			setsAreUniform([reps('5', { weightKg: '80' }), reps('5', { weightKg: '80' })]),
		).toBe(true)
	})
	...
	test('diverging reps, load, or kind break uniformity', () => {
		...
		expect(
			setsAreUniform([reps('5', { weightKg: '80' }), reps('5', { weightKg: '90' })]),
		).toBe(false)
		expect(
			setsAreUniform([reps('5', { weightKg: '80' }), reps('5', { pct1RM: '80' })]),
		).toBe(false)
		expect(
	...
	test('absent and empty fields compare equal; whitespace is ignored', () => {
		expect(setsAreUniform([reps('5'), { kind: 'reps', reps: ' 5 ' }])).toBe(true)
		expect(
			setsAreUniform([reps('5', { weightKg: '' }), { kind: 'reps', reps: '5' }]),
		).toBe(true)
	})
...
describe('switchUniformSetKind', () => {
	test('switching to timed seeds an empty duration and keeps the load', () => {
		expect(
			...
			{ kind: 'timed', reps: '5', durationSec: '30', weightKg: '60', pct1RM: '' },
		])
	})
