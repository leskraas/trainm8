	...
	return (
		<SelectPrimitive.Trigger
			...
			className={cn(
				...
				// zooms the focused control, 14px ≥ md. Touch target (§2.2): the
				// invisible `after:` extension reaches ~44px effective height.
				"bg-input/50 focus-visible:border-ring focus-visible:ring-ring/30 aria-invalid:border-destructive aria-invalid:ring-destructive/20 data-placeholder:text-muted-foreground dark:aria-invalid:border-destructive/50 dark:aria-invalid:ring-destructive/40 relative flex w-fit items-center justify-between gap-1.5 rounded-2xl border border-transparent px-3 py-2 text-base whitespace-nowrap transition-[color,box-shadow] duration-200 outline-none after:absolute after:inset-x-0 after:-inset-y-1.5 focus-visible:ring-3 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:ring-3 data-[size=default]:h-11 data-[size=sm]:h-7 data-[size=sm]:after:-inset-y-2 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 md:text-sm [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
				className,
			)}
			{...props}
		>
...
function SelectItem({
	...
	return (
		...
		>
			...
			>
				<Icon name="check" className="pointer-events-none" aria-hidden="true" />
			</SelectPrimitive.ItemIndicator>
		</SelectPrimitive.Item>
	)
}
