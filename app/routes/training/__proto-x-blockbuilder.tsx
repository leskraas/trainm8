...
export function BlockBuilderVariant({ plan }: { plan: ProtoPlanInput }) {
	...
	// Volume currency: hours stay the stored primitive; km resolves through an
	// easy-run-pace planning assumption (the real thing reads the Discipline
	// Profile), TSS through the per-focus TSS/h assumption.
	...
	const maxTss = Math.max(...weeks.map((w) => displayOf(w.hours, w.focus)), 1)
									...
									>
										...
										>
											...
											<span className="text-muted-foreground ml-auto text-xs tabular-nums">
												{b.weeks} wk · {displayOf(b.hours, b.focus)} {unit} ·{' '}
											...
											<div className="space-y-3 border-t px-3 py-3 text-sm">
												...
												<label className="block">
													<span className="text-xs">
														Volume: {displayOf(b.hours, b.focus)} {unit}/week
														{currency !== 'hours' ? ` (${b.hours} h)` : ''}
													</span>
													<input
														type="range"
														min={2}
								...
								weeks.map((w, i) => {
									...
									return (
										<rect
											key={bar}
											x={padL + bar * colW + 1.5}
											y={yTss(displayOf(w.hours, w.focus))}
											width={colW - 3}
											height={CH - padB - yTss(displayOf(w.hours, w.focus))}
											rx={2}
											fill={FOCUS_META[w.focus].hex}
											opacity={(w.isEasy ? 0.35 : 0.85) * (rep > 0 ? 0.45 : 1)}
										>
											<title>
												Week {i + 1} · {w.blockName} ·{' '}
												{displayOf(w.hours, w.focus)} {unit} ({w.hours} h ·{' '}
												{Math.round(w.hours * KM_PER_HOUR)} km · {w.tss} TSS)
